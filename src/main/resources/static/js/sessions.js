    ...
    await withButtonLoading(button, "Đang tải...", async () => {
        try {
            ...
            if (!Array.isArray(data) || data.length === 0) {
                tbody.innerHTML = `<tr><td colspan="7" class="empty-row">Không có phiên đăng nhập còn hạn.</td></tr>`;
            ...
            tbody.innerHTML = `<tr><td colspan="7" class="empty-row">${escapeHtml(error.message || "Không thể tải phiên đăng nhập.")}</td></tr>`;
...
async function deleteSession(sessionId) {
    ...
    if (!confirm(`Bạn có chắc muốn xóa phiên ${sessionId} không? Token tương ứng sẽ bị logout.`)) {
        return;
    }
